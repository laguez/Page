...
def final(P: np.matrix) : 
    """Cette fonction prend une matrice de transition P en entrée et retourne une matrice finale F.
    Si la somme des éléments de la première colonne de P n'est pas égale à 1, elle retourne la transposée de P.
    Sinon, elle retourne P."""
    if  np.sum(P[:,0]) != 1 :
        return transpose(P)
    else: 
        return P
...
def googlematrix(A : np.matrix , P : np.matrix, alpha : float) : 
    ...
    F = final(P)
    G = (alpha * F) + ((1 - alpha) * ones_matrix(A))
    return G
...
def pageRankPower(A : np.matrix , alpha : float , v : np.array ): 
    ...
    first_four_personalized_vectors(G , v)
